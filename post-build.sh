...
# Post-build script for NervesSystemKoboClaraColour
...
if [ -f "$NERVES_DEFCONFIG_DIR/uboot/boot.cmd" ]; then
    ...
    echo "✓ Compiled board-specific boot.cmd to boot.scr"
else
...
echo "Post-build script completed for nerves_system_kobo_clara_colour"
